...
void do_bgfg(char **argv)
{
    ...
    int pid_number = 0;
    char * ptr_argv = argv[1];
    struct job_t *current_job = NULL;
    if(ptr_argv == '%')
    {
        pid_number = strtol(++ptr_argv,NULL,10);
        if(*ptr_argv == NULL)
        {
            printf("%s command requires PID or %%jobid argument\n", argv[0]);
            return;
        }
        current_job = getjobjid(jobs,pid_number);
        if(!current_job)
        {
        ...
        current_job = getjobpid(jobs,pid_number);
        if(!current_job)
        {
    ...
    if(!strcmp(argv[0], "bg"))
    {
        ...
        if (current_job->state == BG)
        {
            current_job->state = FG;
            waitfg(jobs);
        }
        else if(current_job->state == ST)
        {
            Kill(-(current_job->pid),SIGCONT);
            current_job->state = FG;
        }
